...
class DurationSettings(BaseModel):
    ...
    Os limites de duração seguem a hierarquia:
        hard_lower <= soft_lower <= soft_upper <= hard_upper

    Os limites *soft* representam a duração desejada; os *hard* representam os extremos toleráveis.
    Segmentos fora dos limites hard são descartados ou reparticionados dependendo da etapa de
    pós-processamento.
    ...
    Attributes:
        ...
        hard_lower: Duração mínima tolerável de um segmento, em segundos.
            Segmentos abaixo deste valor são descartados.
        hard_upper: Duração máxima tolerável de um segmento, em segundos.
            Segmentos acima deste valor são descartados.
        overlap: Sobreposição entre segmentos consecutivos, em segundos.
        max_gap: Intervalo máximo entre dois segmentos para que sejam mesclados em um único, em
            segundos.
    """
    ...
    def _validate_duration_hierarchy(self: Self) -> Self:
        """Garante que hard_lower <= soft_lower <= soft_upper <= hard_upper."""

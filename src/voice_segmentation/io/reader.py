"""Leitura de arquivos de áudio para o formato interno."""
...
def load_audio(
    ...
    Args:
        ...
        target_sr: Taxa de amostragem de destino em Hz. Se None, a taxa original do arquivo é
            preservada sem reamostragem.
        mono: Se True, converte sinais multicanal para mono via média dos canais.

    Returns:
        Tupla (audio, sample_rate) onde audio é um array float32 de forma (n_samples,) e
        sample_rate é a taxa de amostragem efetiva após o carregamento.

    Raises:
        FileNotFoundError: Se o caminho não existir no sistema de arquivos.
        ValueError: Se o arquivo não puder ser interpretado como áudio pelo librosa.
    """
    path = Path(path)
    if not path.exists():
